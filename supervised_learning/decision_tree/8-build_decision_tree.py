...
class Node:
    ...
    def update_bounds_below(self):
        """Recursively update bounds below this node."""
        if self.is_root:
            self.upper = {0: np.inf}
    ...
    def pred(self, x):
        """Predict one individual recursively."""
...
class Leaf(Node):
    ...
    def update_bounds_below(self):
        """Stop recursive bounds update at a leaf."""
        pass
...
class Decision_Tree:
    ...
    def np_extrema(self, arr):
        """Return minimum and maximum of an array."""
    ...
    def fit_node(self, node):
        ...
        is_left_leaf = (
            ...
            or np.unique(left_targets).size == 1
        )
        ...
        is_right_leaf = (
            ...
            or node.depth + 1 == self.max_depth
            or np.unique(right_targets).size == 1
        )
    ...
    def Gini_split_criterion_one_feature(
            self, node, feature):
        """Return best threshold and Gini for a feature."""
        values = self.explanatory[:, feature][
            node.sub_population
        ]
        targets = self.target[node.sub_population]

        thresholds = self.possible_thresholds(
            node, feature
        )
        ...
        gini = (
            (left_sizes / total) * left_gini
            + (right_sizes / total) * right_gini
        )

        index = np.argmin(gini)

        return thresholds[index], gini[index]

    def Gini_split_criterion(self, node):
        """Return best feature and threshold using Gini."""
        X = np.array([
